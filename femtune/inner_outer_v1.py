...
class RunNed(RunBase):
    ...
    def enumerate_runs(cls, options):
        ...
        for cl_dev_data in enumerate_distinct_cl_devices():
            ...
            for sd in [3]:  # Kernel only defined in 3d
                for deg in [1, 2, 3]:
                    nbf = pdim_ned(sd, deg)
    ...
    @classmethod
    def get_env_properties(cls, run_props):
        result = super(RunNed, cls).get_env_properties(run_props)
    ...
    @classmethod
    def get_loopy_kernel(cls, run_props, dev):
        from femtune.referencekernels import curlcurlrefknl

        dtype = np.dtype(run_props["prec"])
        knl = curlcurlrefknl(dtype)
        ...
        knl = lp.fix_parameters(knl, nbf=int(nbf), sdim=sd, nqp=nqp)
        refknl = knl
